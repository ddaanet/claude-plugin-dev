...
tagged, pushed, and the GitHub release created. If anything from
...
users, and nothing in the toolkit detected it: the pre-flight in `release` only
compared `plugin.json` against the latest tag, and the version-guard hook only
fires on edits to `plugin.json`.
...
toolkit's own half-landed release, and the same failure shape this check detects
in consumers.
