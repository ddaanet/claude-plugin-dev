...
same question and `--yes` existed only to silence it in the common case where an
outer gate was present — that is, almost always.

Pre-flight guards are unchanged; only the keystroke was removed. The same
applies to this repo's own self-release recipe. See "No interactive confirmation
in `release`" in [design.md](../design.md).
