...
`bump_commit_tag`, rewrote `.claude-plugin/plugin.json` to `0.6.0`, `git add`ed
it, and ran `git commit -m "release: 0.6.0"`. The repo's gitlore pre-commit hook
rejected that commit: memory was dirty with no approved commit summary, so the
hook printed its instructions and exited non-zero. Under `set -euo pipefail` the
script died there.
...
- `just resume-release` refuses — `resume_preflight` requires a local tag at the
  manifest's version, and there is none. Its hint says to run
...
- `just release minor` refuses too, and less helpfully. `common_preflight`'s
  ...
  with a bare `error: uncommitted changes`. Nothing connects that message to the
  script's own leftover, and the natural reading is that the user dirtied the
  tree.
...
`git commit` is the only step in `bump_commit_tag` that can fail against a tree
the preflight already validated, and it is called with no handling, so `set -e`
turns a recoverable refusal into a stranded working tree.
...
2. **Teach `common_preflight` to recognise its own leftover.** If the only
   ...
   `git checkout HEAD -- .claude-plugin/plugin.json` to run. Diagnostic only, no
   state change; composes with option 1 rather than replacing it.

3. **Dry-run the commit gate in `release_preflight`.** Costly and fragile —
   hooks are not generally idempotent or side-effect-free — and it does not help
   when the gate's complaint appears between the check and the commit. Mentioned
   for completeness; I would not take it.
...
1. Make the hook exit non-zero (for gitlore: leave memory dirty with no approved
   summary in `.claude/gitlore-memory-message`).
